...
def initialize_codebook_kmeans(model, dataloader, device, num_samples=2000):
    model.eval()
    all_z = []
    with torch.no_grad():
        for images, _ in dataloader:
            ...
            z_e = model.encoder(x)
            D = z_e.size(1)
            z_flat = z_e.permute(0,2,3,1).reshape(-1, D).cpu()
            all_z.append(z_flat)
            if sum(t.shape[0] for t in all_z) >= num_samples:
                break
    all_z = torch.cat(all_z, dim=0).numpy()
    kmeans = KMeans(n_clusters=model.vq.num_embeddings, random_state=0).fit(all_z)
    centroids = torch.from_numpy(kmeans.cluster_centers_)
    # copy into embeddings
    with torch.no_grad():
        model.vq.embeddings.copy_(centroids.to(model.vq.embeddings.device))
    model.train()
...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, z_e):
        ...
        z_q = self.embeddings[encoding_indices].clone().view(B, H, W, D).permute(0, 3, 1, 2)
...
class VAE(nn.Module):
    def __init__(self, embedding_dim=64, num_embeddings=512, commitment_cost=0.01):
        super().__init__()
